...
namespace imaging {
	...
	Image::Image(unsigned int w, unsigned int h): Array<Vec3<component_t>>(w, h) {
		// nothing more
	}

	Image::Image(const Array<Vec3<component_t>> & source): Array<Vec3<component_t>>( source) {
		// nothing more
	}


	bool Image::operator << (std::string filename) { // Read the contents of an object from the specified file
	...
	bool Image::operator >> (std::string filename)   // Write the contents of an object to the specified file
	{
		// open the file for reading
		ofstream outfile;
	...
	Vec3<component_t> Image::imageAveragePPM() {
